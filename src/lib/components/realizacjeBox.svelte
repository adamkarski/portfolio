	...
	<div style="transform: translateY(50px) translateZ(0px);" in:fade={{ delay: 450 }} out:fade={{duration:150}}>
		<div class="coverImage">
			...
			<!-- {#await preload(src) then _} -->
				<a class="" href="/realizacje/{item.attributes.slug}" in:fade={{ delay: 400 }} draggable={false}>
					<img src="{getUrlImage(item.attributes.miniatura.data.attributes)}" alt={item.attributes.title} draggable={false}/>
				</a>
